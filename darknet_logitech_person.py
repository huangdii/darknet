...
def detect(net, meta, image, thresh=.5, hier_thresh=.5, nms=.45, debug=False):
    ...
    custom_image = cv2.resize(custom_image, (1280, 800))
...
def main(argv):
    ...
    help_str = 'darknet_logitech_person.py -c <config> -w <weight> -m <meta> -t <threshold> -s <svo_file>'
    try:
    ...
    if netMain is None:
        ...
        netMain = load_net(config_path.encode(
            "ascii"), weight_path.encode("ascii"), 0)  # batch size = 1
    if metaMain is None:
...
#    while key != 113:  # for 'q' key
    while True:  # for 'q' key
        ...
        if(True):
            ...
            #image = mat.get_data()
       	    ret, frame = cam.read()
            cv2.namedWindow("KAERI", cv2.WINDOW_NORMAL)
            detections = detect(netMain, metaMain, frame, thresh)
            log.info(chr(27) + "[2J"+"**** " + str(len(detections)) + " Results ****")
            for detection in detections:
                label = detection[0]
                if label != 'person' and label !=  'car' and label != 'bicycle' and label != 'truck' and label != 'cat' : continue
                #if label != 'car': continue
                ...
                thickness = 1
                cv2.rectangle(frame, (x_coord - thickness, y_coord - thickness),
            ...
            cv2.imshow("KAERI", frame)
            cv2.resizeWindow("KAERI", 1280, 800)
            key = cv2.waitKey(5)
